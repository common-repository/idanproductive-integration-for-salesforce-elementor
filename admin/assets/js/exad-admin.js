jQuery(document).ready(function($) {
	...
	$('.exad-dashboard-tab input, .exad-dashboard-tab button, .exad-dashboard-tab-input').on( 'click', function() {
		saveHeaderAction.addClass( 'exad-save-now' );
		saveHeaderAction.removeAttr('disabled').css('cursor', 'pointer');
	} );

	// Sync Button to clear transient cache
	$('.exad-js-element-sync-cache').on('click', function(e) {
		e.preventDefault();

		var $syncButton = $(this);
		var $messageContainer = $('#exad-sync-message');

		$.ajax({
			url: js_exad_settings.ajaxurl,
			type: 'POST',
			data: {
				action: 'elemetix_ajax_clear_cache',
				security: js_exad_settings.ajax_nonce
			},
			beforeSend: function() {
				$syncButton.text('Syncing...').attr('disabled', true);
			},
			success: function(response) {
				if (response.success) {
					$messageContainer.html('<span class="exad-settings-saved exad-sync-success">' + response.data.message + '</span>');
					$messageContainer.find('.exad-sync-success').fadeIn('slow');
					
					setTimeout(function(){
						$messageContainer.find('.exad-sync-success').fadeOut('slow', function() {
							$(this).remove();
						});
					}, 2000);
				} else {
					$messageContainer.html('<span class="exad-sync-error">' + response.data.message + '</span>');
					$messageContainer.find('.exad-sync-error').fadeIn('slow');
				}
			},
			error: function(jqXHR, textStatus, errorThrown) {
				$messageContainer.html('<span class="exad-sync-error">Error synchronizing Salesforce data: ' + textStatus + '</span>');
				$messageContainer.find('.exad-sync-error').fadeIn('slow');
			},
			complete: function() {
				$syncButton.text('Sync').attr('disabled', false);
			}
		});
	});
